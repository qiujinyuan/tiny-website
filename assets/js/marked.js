  ...
  return fetch("https://data.jsdelivr.com/v1/package/npm/marked")
    ...
    .then(function (json) {
      for (var i = 0; i < json.versions.length; i++) {
        ...
        markedVersions[ver] =
          "https://cdn.jsdelivr.net/npm/marked@" + ver + "/lib/marked.js";
    ...
    .then(function () {
      return fetch("https://api.github.com/repos/markedjs/marked/commits")
        ...
        .then(function (json) {
          markedVersions.master =
            "https://cdn.jsdelivr.net/gh/markedjs/marked@" +
            json[0].sha +
            "/lib/marked.js";
        })
        .catch(function () {
          ...
          if (match) {
            switch (match[1]) {
              case "commit":
                addCommitVersion(
                  search.version,
                  match[2].substring(0, 7),
                  match[2]
                );
                return search.version;
              case "pr":
                return getPrCommit(match[2]).then(function (commit) {
                  if (!commit) {
                    return "master";
                  }
  ...
  if (e.which === 13) {
    switch ($markedVerElem.value) {
      ...
      case "pr":
        ...
        getPrCommit(pr).then(function (commit) {
...
function addCommitVersion(value, text, commit) {
  ...
  markedVersions[value] =
    "https://cdn.jsdelivr.net/gh/markedjs/marked@" + commit + "/lib/marked.js";
  var opt = document.createElement("option");
...
function getPrCommit(pr) {
  return fetch(
    "https://api.github.com/repos/markedjs/marked/pulls/" + pr + "/commits"
  )
    .then(function (res) {
      return res.json();
    })
...
function checkForChanges() {
  if (
    ...
    $markedVerElem.value !== "pr" &&
    (typeof marked !== "undefined" || window.Worker)
  ) {
    inputDirty = false;
